var selpayIK = {
  ...
  req_uri : location.origin + '/payment/interkassa/assets/api.php',
  selPaysys : function()
	{
    if(jQuery('button.sel-ps-ik').length > 0)
      jQuery('.sel-ps-ik').click()
    else{
      var form = jQuery('#checkout_confirmation')
  ...
  paystart : function (data) {
    ...
    if (data_array['resultCode'] != 0) {
      ...
      jQuery('#checkout_confirmation').attr('action', selpayIK.actForm).submit()
    }
    else {
      if (data_array['resultData']['paymentForm'] != undefined) {
        ...
        if (document.getElementById('tempdivIK') == null)
          jQuery('#checkout_confirmation').after('<div id="tempdivIK">' + data_array['resultData']['internalForm'] + '</div>');
        else
...
$(document).ready(function(){
  ...
  var checkSelCurrPS = [];var form=jQuery('#checkout_confirmation');
    ...
    if(!jQuery(pm).find('.radioBtn a').hasClass('active')){
      ...
      if(ik_pw_via.search('test_interkassa|qiwi|rbk')==-1){
        ...
        jQuery.post(selpayIK.req_uri+'?nYg=nYa', jQuery('#checkout_confirmation').serialize(), function (data) {
  ...
  jQuery('.radioBtn a').on('click', function () {
    ...
    if (jQuery('input[name ="ik_pw_via"]').length > 0){
      ...
      jQuery('#checkout_confirmation').append(jQuery('<input>', {type: 'hidden', name: 'ik_pw_via', val: ik_pw_via}));
    }
    jQuery.post(selpayIK.req_uri+'?nYg=nYs', jQuery('#checkout_confirmation').serialize())
